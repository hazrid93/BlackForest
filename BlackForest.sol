...
import "./blackforest-imports.sol";
 ...
 * 
 * NOTE: You shouldn't really remove the Rfi fee.
 * 
 */
abstract contract Tokenomics {
    ...
    string internal constant NAME = "BlackForest";
    string internal constant SYMBOL = "BFT";
    ...
    address internal charityAddress = 0xC99fE944194B6F363DcE79FA121d6C3d74399c78;
    address internal marketingAddress = 0x7557E486Ba1E758cD56F6FE5Ed5a4E6E48689818;
     ...
     */
    address internal tipToTheDev = 0xfc2781bd93c120E79DaC1bF96b41324Dc4aac794;
    ...
    function _addFee(FeeType name, uint256 value, address recipient, bool isBuy) private {
        fees.push( Fee(name, value, recipient, 0, isBuy ) );
        sumOfFees += value;
    }
    ...
    function _addFees() private {
         ...
         */ 
         //redistribution fee
        _addFee(FeeType.Rfi, 25, address(this), true ); 
        
        //burn fee
        _addFee(FeeType.Burn, 15, burnAddress, true );
        _addFee(FeeType.Liquidity, 50, address(this), true );
        _addFee(FeeType.External, 15, charityAddress, true );
        _addFee(FeeType.External, 15, marketingAddress, true );

        // 4% to the dev wallet
        _addFee(FeeType.ExternalToETH, 40, tipToTheDev, true );
         ...
         */ 
         //redistribution fee
        _addFee(FeeType.Rfi, 40, address(this), false ); 
        
        //burn fee
        _addFee(FeeType.Burn, 10, burnAddress, false );
        _addFee(FeeType.Liquidity, 50, address(this), false );
        _addFee(FeeType.External, 30, charityAddress, false );
        _addFee(FeeType.External, 30, marketingAddress, false );

        // 4% to the dev wallet
        _addFee(FeeType.ExternalToETH, 40, tipToTheDev, false );
    }
    ...
    function _getFee(uint256 index) internal view returns (FeeType, uint256, address, uint256, bool){
        Fee memory fee = _getFeeStruct(index);
        return ( fee.name, fee.value, fee.recipient, fee.total, fee.isBuy );
    }
...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        _takeFees( amount, currentRate, sumOfFees, sender );
        emit Transfer(sender, recipient, tTransferAmount);
    }
    
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees, address sender ) private {
        if ( sumOfFees > 0 && !isInPresale ){
            _takeTransactionFees(amount, currentRate, sender);
        }
    }
     ...
     */
    function _takeTransactionFees(uint256 amount, uint256 currentRate, address sender) internal virtual;
}
...
abstract contract SafeToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    function _takeTransactionFees(uint256 amount, uint256 currentRate, address sender) internal override {
        ...
        for (uint256 index = 0; index < feesCount; index++ ){
            (FeeType name, uint256 value, address recipient, uint256 total, bool isBuy) = _getFee(index);
            // use BUY tax rate if 'true'
            if(isSenderPair == true){
                // no need to check value < 0 as the value is uint (i.e. from 0 to 2^256-1)
                if ( value == 0 ) continue;
                // if executing buy transfer and FeeType has isBuy == false, skip the sell tax calculation.
                if ( isBuy == false ) continue;
                if ( name == FeeType.Rfi ){
                ...
                // no need to check value < 0 as the value is uint (i.e. from 0 to 2^256-1)
                if ( value == 0 ) continue;
                // if executing sell transfer and FeeType has isBuy == true, skip the buy tax calculation.
                if ( isBuy == true ) continue;
                if ( name == FeeType.Rfi ){
                    _redistribute( amount, currentRate, value, index );
                }
                else if ( name == FeeType.Burn ){

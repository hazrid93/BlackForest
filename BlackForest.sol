...
abstract contract BaseRfiToken is IERC20, IERC20Metadata, Ownable, Presaleable, Tokenomics {
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        _takeFees( amount, currentRate, sumOfFees, isSenderPair );
        emit Transfer(sender, recipient, tTransferAmount);
    }
    
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees, bool isSenderPair ) private {
        if ( sumOfFees > 0 && !isInPresale ){
            _takeTransactionFees(amount, currentRate, isSenderPair);
        }
    }
     ...
     */
    function _takeTransactionFees(uint256 amount, uint256 currentRate, bool isSenderPair) internal virtual;
}
...
abstract contract SafeToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    function _takeTransactionFees(uint256 amount, uint256 currentRate, bool isSenderPair) internal override {
        
        if( isInPresale ){ return; }

        uint256 feesCount = _getFeesCount();
        for (uint256 index = 0; index < feesCount; index++ ){
            (FeeType name, uint256 value, address recipient, bool isBuy) = _getFee(index);
